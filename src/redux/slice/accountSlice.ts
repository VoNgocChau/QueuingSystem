...
export const saveLogToFirestore = createAsyncThunk('log/save', async (logEntry: LogEntry) => {
    try {
        const collection = await firestore.collection('activityLogs').add(logEntry);
...
const accountSlice = createSlice({
    ...
    extraReducers: builder => {
        ...
        }).addCase(saveLogToFirestore.fulfilled, (state, action: PayloadAction<LogEntry>) => {
            state.log.push(action.payload)
        })
    }
})
